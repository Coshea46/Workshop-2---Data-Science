...
```{r Q3}
visitors %>% arrange(desc(n_2022)) # Arranging n_2022 column in descending
```
# Arranging n_2022 column in descending
...
visitors %>%
  filter(
    ...
    admission == "Members",
    n_2022 >= 100000
  )   # Could put count back in but doesn't show which rows satisfy       #conditions
```
...
visitors %>% 
  count(region) %>%
  filter(n>50)   #n representing num of rows (which equals num of #attractions -- can't use attraction>50 since attraction is a String)
```
...
**NA means not avaialable. Getting this error since there are missing values in the n_2021 column (they are marked as NA). **
...
**na.rm is boolean and if evaluates to TRUE, NA values will be stripped from column for computation. If FALSE, will use NA values in computation and hence return NA.**
...
**There are more lower values in the data set**
